...
export function Header({isGroup = false, isAuth = false}: IContentProps) {
  return (
    <header className={styles.header}>
      <Link to='/' className={styles.logo}>menti<span>Ru</span></Link>
      {(isGroup && !isAuth) ? <Group /> : null}
...
const AutoGroup = () => {
  ...
  const {isLoading, setIsAuth} = useContext(AuthContext);
  ...
  return (
    <div className={styles.autoGroup}>
      ...
      <button onClick={() => setIsOpen(!isOpen)} className={styles.user}>
        ...
        <span>{isLoading ? <Skeleton style={{width: "120px", height: "26px"}}/> : "Nikita Sobolev"}</span>
      ...
      {isOpen ? (
        <ul className={styles.list}>
          <li>
            <Link style={{ backgroundImage: 'url(' + user + ')'}} to="/profile">Profile</Link>
            </li>
          <li>
            <Link
                onClick={logout}
...
const Group = () => {
  ...
  return (
    <div className={styles.group}>
      {isLoading ? <Skeleton style={{...style, marginRight: "28px"}}/> : (
